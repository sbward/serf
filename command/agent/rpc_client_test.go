...
// testRPCClient returns an rpc.Client connected to an RPC server that
// serves only this connection.
func testRPCClient(t *testing.T) (*rpc.Client, *Agent, *AgentIPC) {
	...
	rpcClient, err := rpc.NewClient(l.Addr().String())
	if err != nil {
		t.Fatalf("err: %s", err)
	}
